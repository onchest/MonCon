...
1. **프로젝트 목적**
    ...
    DC 모터 기반 소형 회전 장비의 상태를 여러 센서로 모니터링하고, MCU가 정상/경고/고장 상태를 판정하여 모터와 경고 장치를 제어한다. 최종 시스템은 FreeRTOS 기반으로 동작하며, 추후 센서와 액추에이터를 쉽게 추가할 수 있는 구조를 목표로 한다.
...
3. **시스템 상태**
    ...
    ```mermaid
    flowchart TD
        STOPPED -->|Start| NORMAL
        NORMAL -->|이상 징후| WARNING
        NORMAL -->|Critical Condition| FAULT
        WARNING -->|정상 복귀| NORMAL
        WARNING -->|위험 임계값 초과| FAULT
        FAULT -->|Reset & Safe Condition| STOPPED
    ```
...
7. **Bare-metal 구조**
    ...
    - FreeRTOS 구현 이전에 Bare-metal구조로 구현
